...
async.series([
  function(next) {
    ...
    for (var i = 0; i < 100; i++) {
      ...
      // uint64le
      var br = new bivcore.encoding.BufferWriter();
      var num = Math.round(Math.random() * 10000000000000);
      br.writeUInt64LEBN(new bivcore.crypto.BN(num));
      buffers.push(br.toBuffer());

      // hashes
      var data = bivcore.crypto.Hash.sha256sha256(new Buffer(32));
    ...
    function readUInt64LEBN() {
      ...
      var buf = buffers[c];
      var br = new bivcore.encoding.BufferReader(buf);
  ...
  function(next) {
    ...
    function bivcoreTest() {
      block1 = bivcore.Block.fromString(blockData);
    }
    ...
    var suite = new benchmark.Suite();
    suite.add('bivcore', bivcoreTest, {maxTime: maxTime});
