...
Represents a bitcoin value private key and is needed to be able to spend bitcoin value and sign transactions. See the official [Bitcoin Value Wiki]() for more information about private keys. A PrivateKey in Bivcore is an immutable object that has methods to import and export into a variety of formats including [Wallet Import Format]().
...
Note: The WIF (Wallet Import Format) includes information about the network and if the associated public key is compressed or uncompressed (thus the same bitcoin value address will be generated by using this format).
