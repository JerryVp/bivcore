...
const bivcore = require('@sotatek-anhdao/crypto-wallet-core').BivcoreLib;
const crypto = {
  createHash,
  createCipheriv,
  createDecipheriv,
...
export function bivcoinCoreDecrypt(
  jsonl: Array<{
    cipherText?: string;
  ...
  for (let line of jsonl) {
    ...
    if (line.derivationMethod) {
      ...
      let privKey = decrypt({
        key: master,
        iv: bivcore.crypto.Hash.sha256sha256(Buffer.from(line.pubKey, 'hex')),
        cipherText
      });
