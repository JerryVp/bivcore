import { Deriver, Transactions } from '@sotatek-anhdao/crypto-wallet-core';
import * as Bcrypt from 'bcrypt';
import 'source-map-support/register';
import { Client } from './client';
import { Encryption } from './encryption';
import { Storage } from './storage';
const { PrivateKey } = require('@sotatek-anhdao/crypto-wallet-core').BivcoreLib;
const Mnemonic = require('@sotatek-anhdao/bitcore-mnemonic');
...
export class Wallet {
  ...
  constructor(params: Wallet | WalletObj) {
    ...
    if (!this.baseUrl) {
      this.baseUrl = 'https://api.bivcore.io/api';
    }
  ...
  async signTx(params) {
    ...
    if (!keys) {
      ...
      let decryptedParams = Encryption.bivcoinCoreDecrypt(addresses, passphrase);
