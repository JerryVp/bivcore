...
const bivcoreLib = require('@sotatek-anhdao/crypto-wallet-core').BivcoreLib;

export class Storage {
  path: string;
  db: Array<Mongo | Level>;
  collection: 'bivcoreWallets';
  url?: string;
  ...
  async addKeys(params: { name: string; keys: KeyImport[]; encryptionKey: string }) {
    ...
    for (const key of keys) {
      let { pubKey } = key;
      pubKey = pubKey || new bivcoreLib.PrivateKey(key.privKey).publicKey.toString();
      let payload = {};
