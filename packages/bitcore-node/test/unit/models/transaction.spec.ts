...
const bivcoreLib = require('bitcore-lib');
...
describe('Transaction Model', function() {
  ...
  it('should stream all the mint operations', async () => {
    const tx = bivcoreLib.Transaction(TransactionFixture.transaction) as BitcoinTransaction;
    let batches = 0;
  ...
  describe('Wallet Tagging', async () => {
    const tx = bivcoreLib.Transaction(TransactionFixture.transaction) as TaggedBitcoinTx;
    const CURRENT_HEIGHT = 8534;
    ...
    it('should tag the transaction ops, and calculate the fee', async () => {
      function getCoinForInput(i: TransactionInput) {
        ...
        const inputTx = new bivcoreLib.Transaction(fixtureInput) as BitcoinTransaction;
