...
export class CoinModel extends BaseModel<ICoin> {
  ...
  async findOriginalTx(txList: any, mintTxid: string): Promise<any> {
    if (!mintTxid) {
      return null;
    }
    const originalTxs: any = [];
    for (let tx of txList) {
      if (tx.spentTxid === mintTxid) {
        originalTxs.push(tx);
        break;
      }
    }
    if (originalTxs.length > 0) {
      for (let originalTx of originalTxs) {
        const origin = await this.findOriginalTx(txList, originalTx.mintTxid);
        if (!origin) {
          return originalTx;
        } else {
          return origin;
  ...
  async getBalance(params: { query: any }, options: CollectionAggregationOptions = {}) {
    ...
    let unconfirmedIn = 0;
    let unspentUtxos: any = [];
    let spentUtxos: any = [];
    let usedUtxos: any = [];

    const txList = await this.collection.aggregate<any>([{ $match: query }], options).toArray();
    if (txList.length > 0) {
      unspentUtxos = txList.filter(tx => !tx.spentTxid);
      spentUtxos = txList.filter(tx => tx.spentTxid && tx.mintHeight < 0);
      if (unspentUtxos.length > 0) {
        unspentUtxos.forEach(async unspentUtxo => {
          // Calculating the final Unconfirmed balance
          unconfirmedBalance += unspentUtxo.value;

          const originalTx = await this.findOriginalTx(txList, unspentUtxo.mintTxid);
          if (!originalTx && unspentUtxo.mintHeight < 0) {
            unconfirmedIn += unspentUtxo.value;
          }
        });
      }

      if (spentUtxos.length > 0) {
        for (let spentUtxo of spentUtxos) {
          if (usedUtxos.includes(spentUtxo.mintTxid)) {
            continue;
          }
          const originalTx = await this.findOriginalTx(txList, spentUtxo.mintTxid);
          if (!originalTx) {
            unconfirmedIn += spentUtxo.value;
            usedUtxos.push(spentUtxo.mintTxid);
          } else {
            if (usedUtxos.includes(originalTx.mintTxid)) {
              continue;
            }
            if (originalTx.mintHeight > 0) {
              continue;
            }
            unconfirmedIn += originalTx.value;
            usedUtxos.push(originalTx.mintTxid);
          }
        }
      }
    }
    ...
    return result.reduce<{
      ...
      (acc, cur) => {
        ...
        acc.balance = unconfirmedBalance;
        acc.unconfirmed_in = unconfirmedIn;
        acc.unconfirmed_out = acc.confirmed + unconfirmedIn - unconfirmedBalance;
        return acc;
      },
      { confirmed: 0, unconfirmed: 0, unconfirmed_in: 0, unconfirmed_out: 0, balance: 0 }
    );
  }
