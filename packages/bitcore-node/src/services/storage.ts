...
export class StorageService {
  ...
  getFindOptions<T>(model: TransformableModel<T>, originalOptions: StreamingFindOptions<T>) {
    ...
    if (originalOptions.coins) {
      coins = true;
    }
    return { query, options, coins };
  }

  async apiStreamingFind<T>(
    ...
    const { query, options, coins } = this.getFindOptions(model, originalOptions);
    const finalQuery = Object.assign({}, originalQuery, query);

    if (coins) {
      let txs = await model.collection
      ...
      txs.map(tx => {
        let foundOutputUtxo;
        if (tx['spentTxid']) {
          foundOutputUtxo = txs.find(t => t['mintTxid'] === tx['spentTxid']);
          if (!foundOutputUtxo && !txids.includes(tx['spentTxid'])) {
            txids.unshift(tx['spentTxid']);
          }
        }
      });

      let transactions = await txModel.collection
        .find({ chain: finalQuery.chain, network: finalQuery.network, txid: { $in: txids } })
        .addCursorFlag('noCursorTimeout', true)
        .toArray();

      let transaction;
      let times = txids.map(tx => {
        transaction = transactions.find(e => e.txid === tx);
        return transaction ? { txid: tx, time: transaction['blockTime'] / 1000 } : 0;
      });

      times.sort((a, b) => (a['time'] < b['time'] ? 1 : -1));
      txids = times.map(time => {
        return time['txid'];
      });
      ...
      let inputs: any = await model.collection
        .find({ chain: finalQuery.chain, network: finalQuery.network, spentTxid: { $in: txids } })
        .addCursorFlag('noCursorTimeout', true)
        .toArray();

      let outputs: any = await model.collection
        .find({ chain: finalQuery.chain, network: finalQuery.network, mintTxid: { $in: txids } })
        .addCursorFlag('noCursorTimeout', true)
        .toArray();

      return res.json({ txids, times, inputs, outputs });
    } else {
