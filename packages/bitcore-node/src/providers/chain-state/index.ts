...
class ChainStateProxy implements IChainStateProvider {
  ...
  async getNetworkInfo(params: GetNetworkInfoParams) {
    return this.get(params).getNetworkInfo(params);
  }
  ...
  getFaucetHistory(params) {
    return this.get(params).getFaucetHistory(params);
  }

  faucetCoin(params) {
    return this.get(params).faucetCoin(params);
  }
