import { Validation } from '@sotatek-anhdao/crypto-wallet-core';
...
import { FaucetStorage } from '../../../models/faucet';
...
export class InternalStateProvider implements IChainStateService {
  ...
  streamAddressUtxos(params: StreamAddressUtxosParams) {
    ...
    Storage.apiStreamingFind(
      CoinStorage,
      query,
      { limit, since, paging: '_id', direction: Direction.ascending },
      req!,
      res!
    );
  }
  ...
  async getNetworkInfo(params: GetNetworkInfoParams) {
    const { chain, network } = params;
    return this.getRPC(chain, network).getNetworkInfo();
  }
  ...
  async getFaucetHistory(params) {
    let { chain, network } = params;
    const history = await FaucetStorage.collection
      .find({}, { sort: { time: -1 } })
      .addCursorFlag('noCursorTimeout', true)
      .toArray();

    let txids = history.map(tx => {
      return tx['txid'];
    });

    let transactions = await TransactionStorage.collection
      .find({ chain, network, txid: { $in: txids } }, { sort: { blockTime: -1 } })
      .addCursorFlag('noCursorTimeout', true)
      .toArray();
    ...
    history.map((tx, index) => {
      let t: any = {};
      t.txid = tx.txid;
      t.address = tx.address;
      t.value = tx.value;
      t.time = transactions[index].blockTime ? transactions[index].blockTime : tx.time;
      t.fee = transactions[index].fee;
      let blockHeight: any = transactions[index].blockHeight ? transactions[index].blockHeight : -1;
      t.blockHeight = blockHeight;
      if (transactions[index].blockHeight && blockHeight >= 0) {
        t.confirmations = tipHeight - blockHeight + 1;
      } else {
        t.confirmations = -1;
      }
      txs.push(t);
  ...
  async faucetCoin(params) {
    let { chain, network, address, ip, amount } = params;

    const UPPER_BOUND_LIMIT = 5;
    const LOWER_BOUND_LIMIT = 0.1;

    if (!this.isValid({ chain, network, input: address }).isValid) {
    ...
    if (history.length > 0) {
      if (Date.now() - history[0].time.getTime() < 900000) {
        const error = 'Your address/IP has newly faucet, please wait for 15 minutes';
        throw new Error(error);
      }
    }

    let fundedAmount = 0;
    const walletBalance: number = await this.getRPC(chain, network).asyncCall('getbalance', []);
    const availableFaucet =
      walletBalance > 0 ? (walletBalance / 1000 > UPPER_BOUND_LIMIT ? UPPER_BOUND_LIMIT : walletBalance / 1000) : 0;
    const expectAmount = parseFloat(amount) > 0 ? parseFloat(amount) : 0;
    if (LOWER_BOUND_LIMIT <= expectAmount && expectAmount <= availableFaucet) {
      fundedAmount = expectAmount * 10 ** 8;
    } else {
      const error = `Invalid amount=${expectAmount}, the amount has to be set between MIN=${LOWER_BOUND_LIMIT} BIV and MAX=${availableFaucet} BIV`;
      throw new Error(error);
    }

    const result: string = await this.getRPC(chain, network).asyncCall('sendtoaddress', [address, expectAmount]);
    if (result) {
      await FaucetStorage.addFaucetRecord({
        chain,
        network,
        txid: result,
        address,
        ip,
        value: fundedAmount.toString()
      });
    }
    return result;
  }
