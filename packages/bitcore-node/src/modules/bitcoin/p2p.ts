...
export class BitcoinP2PWorker extends BaseP2PWorker<IBtcBlock> {
  protected bivcoreLib: any;
  protected bivcoreP2p: any;
  ...
  constructor({ chain, network, chainConfig, blockModel = BitcoinBlockStorage }) {
    ...
    this.bivcoreLib = Libs.get(chain).lib;
    this.bivcoreP2p = Libs.get(chain).p2p;
    ...
    this.invCache = {};
    this.invCacheLimits = {
      [this.bivcoreP2p.Inventory.TYPE.BLOCK]: 100,
      [this.bivcoreP2p.Inventory.TYPE.TX]: 100000
    };
    this.messages = new this.bivcoreP2p.Messages({
      network: this.bivcoreLib.Networks.get(this.network)
    });
    this.pool = new this.bivcoreP2p.Pool({
  ...
  setupListeners() {
    ...
    this.pool.on('peertx', async (peer, message) => {
      ...
      if (this.isSyncingNode && !this.isCachedInv(this.bivcoreP2p.Inventory.TYPE.TX, hash)) {
        this.cacheInv(this.bivcoreP2p.Inventory.TYPE.TX, hash);
    ...
    this.pool.on('peerblock', async (peer, message) => {
      ...
      const blockInCache = this.isCachedInv(this.bivcoreP2p.Inventory.TYPE.BLOCK, hash);
      if (!blockInCache) {
        block.transactions.forEach(transaction => this.cacheInv(this.bivcoreP2p.Inventory.TYPE.TX, transaction.hash));
        this.cacheInv(this.bivcoreP2p.Inventory.TYPE.BLOCK, hash);
      }
    ...
    this.pool.on('peerinv', (peer, message) => {
      if (this.isSyncingNode) {
        const filtered = message.inventory.filter(inv => {
          const hash = this.bivcoreLib.encoding
