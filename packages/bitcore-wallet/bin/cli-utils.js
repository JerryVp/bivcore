...
var Client = require('@sotatek-anhdao/bitcore-wallet-client').default;
...
var die = Utils.die = function(err) {
  if (err) {
    if (err.code && err.code == 'ECONNREFUSED') {
      console.error('!! Could not connect to Bivcore Wallet Service');
    } else {
...
Utils.configureCommander = function(program) {
  program
    ...
    .option('-h, --host <host>', 'Bivcore Wallet Service URL (eg: http://localhost:3232/bws/api')
