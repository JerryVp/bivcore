...
export class TransactionListComponent implements OnInit {
  ...
  public populateTxsForAddress(txidCoins) {
    _.forEach(txidCoins.txids, (txid: any) => {
      ...
      tx.hasUnconfirmedInputs = _.some(tx.vin, input => {
        return input.mintHeight < 0;
      });
      tx.blockheight = tx.vout[0].mintHeight;
      tx.time = new Date(txidCoins.times[txidCoins.txids.indexOf(txid)]).getTime();
      tx.valueOut = tx.vout
  ...
  public fetchAddressTxCoinInfo() {
    this.addrProvider
      .getAddressActivityCoins(this.queryValue, this.chainNetwork)
      .subscribe(addrTxs => {
          this.populateTxsForAddress(addrTxs);
