...
import { BitcoreLib, BitcoreLibCash, BitcoreLibValue } from '@sotatek-anhdao/crypto-wallet-core';

var Bitcore = BitcoreLib;
var BCHAddress = BitcoreLibCash.Address;
var BIVAddress = BitcoreLibValue.Address;
...
export class Verifier {
  ...
  static checkPaypro(txp, payproOpts) {
    ...
    if (
      txp.coin == 'biv' &&
      new BIVAddress(toAddress).toString() != new BIVAddress(payproOpts.instructions[0].toAddress).toString()
    )
      return false;
    if (
      txp.coin == 'biv' &&
