...
import * as CWC from '@sotatek-anhdao/crypto-wallet-core';
...
var Bitcore_ = {
  ...
  biv: CWC.BitcoreLibValue,
  eth: CWC.BitcoreLib,
  xrp: CWC.BitcoreLib,
};
var Mnemonic = require('@sotatek-anhdao/bitcore-mnemonic');
var url = require('url');
...
export class API extends EventEmitter {
  ...
  static BitcoreValue = CWC.BitcoreLibValue;
  ...
  // *
  // * @param {string} coin - 'btc' (default) or 'biv'
  ...
  getFeeLevels(coin, network, cb) {
    ...
    this.request.get(
      '/v2/feelevels/?coin=' + (chain || 'biv') + '&network=' + (network || 'livenet'),
      (err, result) => {
        if (err) return cb(err);
  ...
  // * @param {string} opts.coin[='btc'] - The coin for this wallet (btc, bch, biv).
  ...
  // * @param {String} copayerName
  // * @param {Object} opts
  // * @param {string} opts.coin[='btc'] - The expected coin for this wallet (btc, bch, biv).
  ...
  static serverAssistedImport(opts, clientOpts, callback) {
    ...
    let sets = [
      ...
      {
        // older BTC/BCH/BIV  multisig wallets: /[44]/[0,145]'/
        nonCompliantDerivation: false,
        useLegacyCoinType: false,
